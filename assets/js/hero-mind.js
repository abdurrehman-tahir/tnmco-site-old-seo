...
(function () {
    ...
    var N_SPHERE = mobile ? 700 : 1500;
    var N_RING   = mobile ? 260 : 520;
    ...
    for (var k = 0; k < N_RING; k++) {
        ring.push({
            ...
            r: 1.18 + Math.pow(Math.random(), 1.6) * 0.55,  // radius (sphere=1)
            y: (Math.random() - 0.5) * 0.06,                // band thickness
    ...
    function tint(depth, alpha) {
        var r = Math.round(lerp(122, 16, depth));
        var g = Math.round(lerp(86, 150, depth));
        var b = Math.round(lerp(196, 205, depth));
    ...
    function draw() {
        ...
        ctx.strokeStyle = "rgba(27,177,220," + (0.34 * (1 - pulse)) + ")";
        ctx.lineWidth = 1.3;
        ctx.stroke();
        ...
        function ringPoint(p, move) {
            ...
            var ty = y * rc - z * rs * 0.32, tz = z * rc + y * rs;     // tilt band
            var rx = x * cosY - tz * sinY, rz = x * sinY + tz * cosY; // spin with scene
            return { x: rx, y: ty, z: rz };
        }
        var frontRing = [];
        for (var q = 0; q < N_RING; q++) {
            ...
            var d = (rp.z + 1.8) / 3.6;
            ctx.fillStyle = tint(d, 0.16 + d * 0.3);
            var sz = ring[q].s * lerp(0.9, 1.7, d);
        ...
        for (var n = 0; n < N_SPHERE; n++) {
            ...
            var size = p.s * lerp(0.8, 2.5, depth);
            ctx.fillStyle = tint(depth, 0.14 + depth * 0.62);
        ...
        for (var c = 0; c < 26; c++) {
            ...
            var cr = R * 0.085 * (0.4 + 0.6 * Math.abs(Math.sin(c * 2.4 + t * 0.0009)));
        ...
        ctx.beginPath();
        ctx.arc(CX, CY, 4.5 + beat * 3, 0, 6.2832);
        ctx.fillStyle = "rgba(27,177,220," + (0.75 + beat * 0.25) + ")";
        ctx.fill();
        ...
        for (var fq = 0; fq < frontRing.length; fq++) {
            ...
            var fd = (fr.z + 1.8) / 3.6;
            ctx.fillStyle = tint(fd, 0.2 + fd * 0.5);
            var fsz = rpart.s * lerp(0.9, 1.9, fd);
            ctx.fillRect(CX + fr.x * R - fsz / 2, CY + fr.y * R - fsz / 2, fsz, fsz);
        }
        // comet with trail
        for (var tr = 0; tr < 16; tr++) {
            var cp = ringPoint({ a: t * 0.00085 - tr * 0.035, r: 1.45, y: 0, sp: 0 }, false);
            var ta = (1 - tr / 16);
            var front = cp.z > 0 ? 1 : 0.45;
            ctx.beginPath();
            ctx.arc(CX + cp.x * R, CY + cp.y * R, (3.2 - tr * 0.16) * front, 0, 6.2832);
            ctx.fillStyle = "rgba(27,177,220," + (ta * 0.8 * front) + ")";
            ctx.fill();
        }
